...
const modalStatisticsContent = document.querySelector(
  ".modal-statistics-content"
);
const graphs = document.querySelector(".graphs");
const modalControlPopulation = document.querySelector(
  "#modal-control-population"
);
const modalControlLanguage = document.querySelector("#modal-control-language");
const chartPopulationContainer = document.querySelector(
  ".population-chart-container"
);
const chartLanguageContainer = document.querySelector(
...
const showCountries = (arr) => {
  arr.forEach((country) => {
    let { name, code, capital, languages, flag, population } = country;
    const countryDiv = document.createElement("div");
    countryDiv.classList.add("country");
    countryDiv.innerHTML += `<div><span>${getFlagFromEmoji(code)}</span></div>`;
  ...
  message.innerHTML =
    arr.length > 1
      ? `There are <span class="amount">${arr.length}</span> countries on this
  list.`
      : `There is only <span class="amount">${arr.length}</span> country on this
  list.`;
...
const filterCountriesBySearchTerm = (arr, searchTerm) => {
  let filtered = arr.filter((country) => {
    ...
    let match =
      ...
      languages.join(",").toLowerCase().includes(searchTerm.toLowerCase());
    return match;
  });
...
const buttonSiblings = (buttonName) =>
  [...buttonName.parentElement.children].filter(
    (currentButton) =>
      currentButton.nodeType === 1 && currentButton != buttonName
  );
...
const sortByName = () => {
  ...
  countriesContainer.innerHTML = "";
  showCountries(
    filterCountriesBySearchTerm(countriesCopy, inputField.value).sort(
      (a, b) => {
        if (a.name < b.name) {
          return 1;
        }
...
const tenMostPopulous = entirePopulationDataCopy
  .sort((a, b) => {
    if (a.population < b.population) {
      return 1;
    }
...
worldPopulation = entirePopulationData.reduce(
  (total, element) => total + element.population,
  0
);
...
const buildChartsPopulation = (arr) => {
  arr.forEach((element) => {
    ...
    const population =
      element.population >= 1000000000
        ? element.population.toString().slice(0, 1) +
          "." +
          element.population.toString().slice(1, 3) +
          "B"
        : element.population.toString().slice(0, 3) + "M";
...
uniqueLanguagesArray.forEach((item1) => {
  let languageMatchInBothArrays = entireLanguageData.filter((item) => {
    return item1 === item;
  });
...
uniqueLanguagesArray.forEach(
  (language, index) => (languageNumberOfTimes[language] = numberLang[index])
);

const languageNumberOfTimesArrayOfArrays = Object.entries(
  languageNumberOfTimes
);
const tenMostSpokenLanguages = languageNumberOfTimesArrayOfArrays
  .slice()
  .sort((a, b) => {
    if (a[1] < b[1]) {
      return 1;
